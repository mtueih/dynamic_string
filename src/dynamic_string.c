...
// 动态调整一个「动态字符串」的容量
static bool capacity_resize_dynamic(dstr_adt *const dstr, const size_t new_len) {
	char *new_cstr;
	size_t new_cap;
	size_t adjusted_cap;
	...
	new_cap = new_cap > dstr->min_cap
			  ? new_cap
			  : dstr->min_cap;

	// new_cap 为 1，表示字符串长度为空，不必单独存储 '\0'，释放内存
	if (new_cap == 1) {
	...
	// 如果新容量与当前容量相同，则直接返回 true
	if (new_cap == dstr->cap) {
		return true;
	}

	// 如果新容量小于当前容量，则延迟减容，仅当新容量小于当前容量的 1/4 时，才减容
	if (new_cap < dstr->cap && new_cap > dstr->cap >> 2) {
		return true;
	}

	// 计算与缓存行对齐的容量
	adjusted_cap = align_up(new_cap, DSTR_CACHELINE_SIZE);

	// 分配向上对齐到缓存行的整数倍的容量
	new_cstr = (char *) realloc(dstr->data, adjusted_cap);
	if (new_cstr == NULL) goto memory_allocation_successful;

	if (adjusted_cap <= new_cap) return false;

	// 分配向上对齐到指针大小的整数倍的容量
	if (sizeof(void *) < DSTR_CACHELINE_SIZE) {
		adjusted_cap = align_up(new_cap, sizeof(void *));
...
int dstr_cat_sub_cstr(dstr_adt *const dest, const char *const src, const size_t sub_index, const size_t sub_count) {
	...
	if (!capacity_resize_dynamic(dest, dest->len + sub_count)) {
		return DSTR_MEMORY_ALLOC_FAILED;
	}
